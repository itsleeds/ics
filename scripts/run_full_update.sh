...
python3 scripts/04_run_extract.py --engine gemini --model gemini-3.1-flash-lite 2>&1 | tee -a "$LOGFILE"
...
git add results/results_flat.csv results/results.json results/extracted/[0-9]*.json
git commit -m "feat(pipeline): complete full-corpus re-extraction run with rate-limit immune pacing (schema 2026.3)" || true
